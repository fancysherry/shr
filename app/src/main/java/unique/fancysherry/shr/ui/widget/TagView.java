...
public class TagView extends TextView {
    ...
    if (attrs != null) {
      TypedArray attributesArray =
          context.getTheme().obtainStyledAttributes(attrs,
              R.styleable.TagView, defStyle, R.style.Widget_TagView);
      tagPadding =
          attributesArray.getDimensionPixelSize(R.styleable.TagView_tagPadding,
              dipToPixels(DEFAULT_PADDING));
      tagCornerRadius =
          attributesArray.getDimensionPixelSize(R.styleable.TagView_tagCornerRadius,
              dipToPixels(DEFAULT_CORNER_RADIUS));
      uppercaseTags =
          attributesArray.getBoolean(R.styleable.TagView_tagUppercase, DEFAULT_UPPERCASE);
  ...
  private static class TagSpan extends ImageSpan {
    public TagSpan(String text, int tagPadding, float textSize, boolean bold, int textColor,
        int tagColor, float roundCornersFactor) {
      super(new TagDrawable(text, tagPadding, textSize, bold, textColor, tagColor,
          roundCornersFactor));
    }
  }
  ...
  private static class TagDrawable extends Drawable {
    ...
    public TagDrawable(String text, int tagPadding, float textSize, boolean bold, int textColor,
        int tagColor, float roundCornersFactor) {
      ...
      setBounds(0, 0,
          (int) textContentPain.measureText(text) + backgroundPadding.left
    ...
    public void draw(Canvas canvas) {
      ...
      canvas.drawText(text, backgroundPadding.left + MAGIC_PADDING_LEFT,
          textContentPain.getTextSize() + backgroundPadding.top - MAGIC_PADDING_BOTTOM,
          textContentPain);
    }
