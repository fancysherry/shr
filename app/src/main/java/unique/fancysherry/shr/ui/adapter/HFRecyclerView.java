...
public class HFRecyclerView extends RecyclerView {
  ...
  public void addHeaderView(View view, Object data) {
    final HeaderViewRecyclerAdapter.FixedViewInfo info =
        new HeaderViewRecyclerAdapter.FixedViewInfo();
    info.view = view;
    info.data = data;
    ...
    if (getAdapter() != null) {
      if (!(getAdapter() instanceof HeaderViewRecyclerAdapter)) {
        setAdapterInParentClass(new HeaderViewRecyclerAdapter(mHeaderViewInfos, mFooterViewInfos,
            getAdapter()));
      }
